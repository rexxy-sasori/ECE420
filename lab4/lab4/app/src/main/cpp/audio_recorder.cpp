...
void AudioRecorder::ProcessSLCallback(SLAndroidSimpleBufferQueueItf bq) {
    ...
    // On completion, write your changes back to dataBuf and push it onto the recording queue for
    // audio_player.cpp to pick up
    ...
    while (freeQueue_->front(&freeBuf) && devShadowQueue_->push(freeBuf)) {
        freeQueue_->pop();
        (*bq)->Enqueue(bq, freeBuf->buf_, freeBuf->cap_);
